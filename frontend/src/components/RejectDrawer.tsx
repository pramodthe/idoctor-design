  ...
  if (rejects.length === 0) {
    return (
      <div className="border-t border-red-200/60 pt-4">
        ...
        <p className="text-xs text-slate-400">No designs rejected in this run.</p>
      </div>
    );
  }
      ...
      <ul className="space-y-3">
        {rejects.map((v) => {
          ...
          return (
            <li
              key={v.subject_id}
              className="border-l-2 border-red-500/70 bg-red-50/40 py-2 pl-3 pr-2"
            >
              ...
              <div className="mt-1.5 flex flex-wrap gap-1">
                {v.reasons.map((r) => (
                  <span
                    key={r}
                    className="rounded border border-red-200 bg-white/80 px-1.5 py-0.5 font-mono text-[10px] text-red-800"
                  >
                    {r}
                  </span>
                ))}
